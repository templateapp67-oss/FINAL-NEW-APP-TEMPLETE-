...
--
-- Live SQL editor runs as `postgres`, which is NOT the owner of
-- storage.objects (`supabase_storage_admin` is). ALTER / GRANT / POLICY
-- on that table raise 42501 and would abort the whole BEGIN…COMMIT.
-- Those statements run only when the current role owns the table.
-- The helper function and the salon-media bucket row still apply.
-- ===========================================================================

do $do_salon_media_bucket$
begin
...
exception
  when insufficient_privilege then
    raise notice 'M38: skipping storage.buckets upsert (%)', sqlerrm;
end
$do_salon_media_bucket$;
...
-- Tenant-scoped object policies + grants. Skipped when we do not own
-- storage.objects (SQL editor / live). Applied on a fresh bootstrap
-- where the migration role created the table.
do $do_media_policies$
declare
...
begin
  ...
  if not coalesce(v_is_owner, false) then
    raise notice
      'M38: skipping storage.objects DDL; current_user % is not the table owner',
      current_user;
    return;
  end if;
