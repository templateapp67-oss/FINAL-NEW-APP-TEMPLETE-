-- M57 LIVE APPLY — Supabase SQL Editor (project qwaehqsmodekbgvnaavz)
--
-- यह script legacy `/royal-hair-studio` showcase को किसी real owner के
-- organization से अलग dedicated, unowned organization में move करता है.
-- Salon UUID, public slug, website config, publish state और salon-keyed data
-- preserve रहते हैं; कोई user salon delete/unpublish/rename नहीं होता.
--
-- APPLY ORDER: M54 पहले applied होना चाहिए. पूरा file एक बार Run करें.
-- फिर verify करें:
--   select check_name, ok, detail
--   from public.verify_m57_showcase_tenant_detachment()
--   order by check_name;
-- Expected: सभी 4 rows में ok = true.
--
-- यह file tracked migration के exact transaction body को नीचे embed करती है.
