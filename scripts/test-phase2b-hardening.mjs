 ...
 * M28–M34), seeds legacy rows, then executes the eight required final
...
for (const file of [...PHASE1A_FILES, ...PHASE2_FILES, ...PHASE2A_FILES, ...PHASE2B_FILES]) {
...
await test('themes — five canonical themes, unique slugs, canonical family slug kept', async () => {
  ...
  // The brief's 'full_service_family_salon' wording is an example name; the
  // established canonical slug is 'family_full_service' (M28/M32 seeds and
  // both repositories' type layers). Keep it, enforce uniqueness.
  const family = themes.find((t) => t.theme_id === 'family_full_service');
  assert.equal(family.slug, 'family_full_service');
...
await test('idempotency — M34 replays cleanly on the hardened schema', async () => {
  const sql = await readFile(join(migrationDir, PHASE2B_FILES[0]), 'utf8');
  await db.exec(sql);
});
...
await db.close();
console.log(`Phase 2B hardening tests: ${passed} passed`);
