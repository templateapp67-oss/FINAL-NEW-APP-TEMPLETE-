...
// M55-M67 in timestamp order. M59-M62 came from main; M63/M64 from that
// branch; M65 adds home-service bookings; M66 adds owner-photo public
// parity; M67 fixes saved-service re-add ("already saved" on archived
// rows) with upsert/revive semantics. Asserted by position so a
// re-ordering or a dropped file fails loudly.
assert.ok(existing.files.at(-13).includes('_m55_'));
assert.ok(existing.files.at(-12).includes('_m56_'));
assert.ok(existing.files.at(-11).includes('_m57_'));
assert.ok(existing.files.at(-10).includes('_m58_'));
assert.ok(existing.files.at(-9).includes('_m59_'));
assert.ok(existing.files.at(-8).includes('_m60_'));
assert.ok(existing.files.at(-7).includes('_m61_'));
assert.ok(existing.files.at(-6).includes('_m62_'));
assert.ok(existing.files.at(-5).includes('_m63_'));
assert.ok(existing.files.at(-4).includes('_m64_'));
assert.ok(existing.files.at(-3).includes('_m65_'));
assert.ok(existing.files.at(-2).includes('_m66_'));
assert.ok(existing.files.at(-1).includes('_m67_'));
ok('Design-A M01-M27 and canonical M28-M67 tracks cannot be conflated');
